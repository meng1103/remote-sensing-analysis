...
dataset_path = 'data/data134796/dataset'
...
train_transforms = T.Compose([
    # 随机裁剪
    T.RandomCrop(
        crop_size=CROP_SIZE,         # 裁剪区域将被缩放到此大小
        aspect_ratio=[1.0, 1.0],     # 将裁剪区域的横纵比固定为1
        scaling=[0.2, 1.0]           # 裁剪区域相对原始影像长宽比例在一定范围内变动，最小不低于原始长宽的1/5
    ),

    T.BatchRandomResizeByShort(
        short_sizes=[512, 512]
    ),
...
model.train(
    ...
    use_vdl=True,
    # 指定从某个检查点继续训练
    resume_checkpoint='/home/aistudio/exp/epoch_550'
)
...
# 静态模型导出到 inference_model 文件夹下
# python /home/aistudio/PaddleRS/deploy/export/export_model.py --model_dir=/home/aistudio/exp/best_model/ --save_dir=/home/aistudio/inference_model/  --fixed_input_shape=[1024,1024]